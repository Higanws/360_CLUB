...
| **Socio (member)** | Solo el **portal del socio** (dieta y rutina semanal), no el menú de gestión. |
...
Desde ahí pueden ver:

- **Dieta** — plan nutricional asignado.
- **Rutina** — entrenamiento de la semana (snapshot según asignación y semana en curso).
...
- Despliegue en servidor: ver [DEPLOYMENT.md](./DEPLOYMENT.md).
- Arranque en desarrollo: desde la carpeta `Recreacion_react`, `npm run api` y `npm run web`.
