...
/**
 * Seed mínimo para desarrollo (`npm run db:seed`).
 * El asistente de instalación **no** usa este archivo: aplica el baseline Prisma con datos demo integrados.
 */
const prisma = new PrismaClient();

/** Mismos hashes bcrypt que la migración baseline Prisma (admin / staff demo). */
const HASH_ADMIN =
...
async function clearMvpData(): Promise<void> {
  await prisma.$transaction([
    prisma.clubAccessLog.deleteMany(),
    prisma.memberWeeklyRoutine.deleteMany(),
    prisma.nutritionPlan.deleteMany(),
    prisma.trainingAssignmentTrainer.deleteMany(),
    prisma.trainingAssignmentMember.deleteMany(),
    prisma.trainingAssignment.deleteMany(),
    prisma.trainingRoutineActivity.deleteMany(),
    prisma.trainingRoutine.deleteMany(),
    prisma.activityTrainer.deleteMany(),
    prisma.activityVideo.deleteMany(),
